...
import {arrayOFLabelIDs, resetArrayOfLabelIDs} from "./form/selectLabel.js";
...
function controlAndShowDate(task) {
    // convert selected date to timestamp
    let response = (task.deadline_date) ? controlDate(new Date(task.deadline_date) / 1000) : '';

    if (response === '') {
        return `<span class="flex items-center" id="task-deadline-${task.id}"></span>`;
    } else {
        return `<span class="flex items-center ${response.color}" id="task-deadline-${task.id}" title="${DAY_NAMES[response.date.getDay()]} ${response.date.getDate()} ${MONTH_NAMES[response.date.getMonth()]} ${response.date.getFullYear()}">${calendar_dot_4 + response.day}</span>`
    }
}
                            ...
                            <div class="p-1">
                                <div x-data="datepickerSave(${task.id}, '${task.deadline_date}')" x-init="[initDate(), initDatepicker()]">
                                    <div class="text-xs">
                                        ...
                                        <button class="w-full rounded text-left px-3 py-1 hover:bg-gray-100 flex justify-between" type="button" @click="chooseDateShortcut('this_weekend')">
                                        ...
                                        <button class="w-full rounded text-left px-3 py-1 hover:bg-gray-100 flex justify-between" type="button" @click="chooseDateShortcut('next_week')">
                                        ...
                                        <button class="w-full rounded text-left px-3 py-1 hover:bg-gray-100 flex items-center font-bold" type="button" @click="chooseDateShortcut('no_date')" x-show="showNoDate">
                                            <svg class="w-5 h-5 mr-2 text-gray-800" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M12 3a9 9 0 110 18 9 9 0 010-18zm0 1a8 8 0 100 16 8 8 0 000-16zm3.854 4.146a.5.5 0 010 .708l-7 7a.5.5 0 01-.708-.708l7-7a.5.5 0 01.708 0z"></path></svg>
                                            No Date
                                        </button>
                                    </div>
                                    <hr class="my-2">
